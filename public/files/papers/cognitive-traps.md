...
*Four-column flow diagram showing the framework as a four-step cycle with feedback loop. Step 1 (Identify Constraints): search CS benchmarks for model limitations, prioritize large human-model performance gaps, validate across multiple model architectures. Constraints identified: training data overfitting, spatial reasoning, spatiotemporal reasoning, cross-modal binding, and compositional counting. Step 2 (Design Trap Stimuli): transform constraints into visual survey items, ensure visual (not text) stimulus presentation, design diagnostic error patterns for classification. Implementations: modified illusions, trajectory prediction, object binding tasks. Step 3 (Validate: Models and Humans): test across multiple frontier models, validate with human participants. Suggested thresholds: >75% human accuracy, >30pp human-model gap. Results: 34 models, 2,040 trials, 171 human participants, 1 of 7 traps excluded. Step 4 (Deploy in Survey): embed traps in survey alongside study measures, pre-register threshold before data collection, flag responses exceeding the failure threshold. Results: 3+ failures of 6 traps = 97.1% detection, 4.1% false positive rate. A dashed arrow from Step 4 loops back to Step 1, labeled "Iterate: new AI evaluation research continuously documents new constraints."*

*Note: Figure values extracted from source analysis code (ground truth). Figure image not included in machine-readable version. See published paper for the visual.*
...
*Scatter plot showing average cognitive trap pass rate (%) on the y-axis versus model release date on the x-axis, spanning March 2024 (month 0) through February 2026 (month 25). Each dot represents one of 34 vision-language models, colored by provider: Anthropic (rust red, #c44e2d), OpenAI (green, #3a7d5e), Google (blue, #2c5f7c). Key frontier models are labeled with larger dots and bold text. A dashed red horizontal line marks the 86.8% human baseline (average across 171 attentive participants on the six retained traps). A gray OLS trend line is essentially flat (slope = +0.51 pp/month, p = .282), indicating no statistically significant improvement over nearly two years. Labeled models include (from left to right): Claude Haiku 3.0 (~40%), GPT-4o (~0%), Claude Haiku 3.5 (~45%), Gemini 2.5 Pro (~15%), GPT-4.1 (~8%), GPT-5 with thinking (~28%), Claude Sonnet 4.5 (~23%), Claude Opus 4.5 (~17%), Gemini 3 Pro (~27%), GPT-5.2 with thinking (~45%), GPT-5.2 Pro (~40%), Claude Opus 4.6 (~33%), and Gemini 3 Flash with thinking (~65%, the best model, still 21.8 pp below human baseline). The x-axis tick marks are Jan '24, Jul '24, Jan '25, Jul '25, Jan '26. The y-axis ranges from 0% to 100%. No model passes all six traps.*
...
Westwood, Sean J. (2025), "[The Potential Existential Threat of Large Language Models to Online Survey Research](https://doi.org/10.1073/pnas.2518075122)," *Proceedings of the National Academy of Sciences*, 122(47), e2518075122.
...
**1) DATA QUALITY THREATS IN ONLINE SURVEYS**

**1) THE COGNITIVE TRAP FRAMEWORK: FOUNDATIONS AND IMPLEMENTATION**

2\) Step 1: Identify Architectural Constraints

2\) Step 2: Design Survey-Compatible Implementations (Cognitive Traps)

2\) Step 3: Validate Against Models and Humans

2\) Step 4: Deploy in Research Context

**1) DETECTING AI AGENTS IN ONLINE RESEARCH: VALIDATION ON PROLIFIC**

2\) Method

*3) Participants (Humans)*

*3) Participants (Machines)*

*3) Procedure*

2\) Results

*3) Cognitive Trap Detection Performance*

*3) Individual Cognitive Traps*

*3) CAPTCHA Comparison*

*3) Diagnosticity Simulations*

*3) Choice Deferral Results*

**1) REPLICATION ON AMAZON MTURK AND CLOUDRESEARCH CONNECT**

2\) Amazon Mechanical Turk Results

2\) CloudResearch Connect Results
...
2\) Current Prevalence and the Economics of AI Survey Fraud
...
[^1]: Unlike Tommy's scenario, this is a hypothetical example, but one we (researchers who rely on online surveys) must be prepared for.

[^2]: Researchers are encouraged to submit (i) their results using existing cognitive traps, and (ii) new cognitive traps to the repository.
...
*Horizontal bar chart comparing agent detection rates (%) across nine methods, ordered from highest to lowest. Bars are colored green for the top method and red for all others. The exact detection rates plotted are: Cognitive Traps (3+ failures) = 97.1% (green bar); CAPTCHA (optimal threshold 0.90) = 93.9%; CAPTCHA (standard 0.5) = 4.6%; Traditional Methods (any failure) = 2.3%; Straight-lining = 1.3%; Memory Check = 0.8%; IMC = 0.2%; Obvious Answer = 0.2%; Fast Completion = 0.2%. The x-axis runs from 0% to 105%. Values above 5% are displayed inside the bar in white bold text; values at or below 5% are displayed outside the bar in red bold text. The y-axis is inverted so the best-performing method appears at the top.*
...
*Paired bar chart with six cognitive traps on the x-axis and pass rate (%) on the y-axis (range 0-108%). For each trap, a blue bar shows the human pass rate and a red bar shows the agent pass rate, with exact percentage labels above each bar. Traps are ordered by discrimination (human-agent gap). The exact values plotted are: Muller-Lyer: 92.8% human, 5.9% agent (86.9 pp gap). Cafe Wall: 88.4% human, 5.1% agent (83.2 pp gap). Moving Robot: 77.9% human, 0.4% agent (77.5 pp gap). Ebbinghaus: 95.6% human, 22.4% agent (73.2 pp gap). Surrounded Planets: 96.5% human, 44.5% agent (52.0 pp gap). Colliding Oranges: 88.7% human, 49.4% agent (39.2 pp gap). A horizontal grid provides reference lines. Human pass rates (blue, #2c7bb6) cluster between 77.9% and 96.5%, while agent pass rates (red, #d7191c) range from 0.4% to 49.4%. All differences significant at p < .001.*
...
*Grouped bar chart with cognitive trap scores (0-6 correct) on the x-axis and percentage of agents on the y-axis (range 0-62%). Four platforms are shown as grouped bars at each score value, with bar width 0.2. A dashed vertical line at x = 2.5 marks the 3-failure detection threshold (scores 0-2 are flagged). The exact score distributions plotted for each platform are: Google Project Mariner (M = 0.50, blue #4285F4): score 0 = 56.7%, score 1 = 36.6%, score 2 = 6.7%, scores 3-6 = 0.0%. Claude Sonnet 4.5 via Cursor (M = 0.99, red #c44e2d): score 0 = 34.8%, score 1 = 37.9%, score 2 = 20.5%, score 3 = 6.8%, scores 4-6 = 0.0%. ChatGPT Agent (M = 1.76, teal #10A37F): score 0 = 4.7%, score 1 = 36.4%, score 2 = 45.0%, score 3 = 7.8%, score 4 = 4.7%, score 5 = 1.6%, score 6 = 0.0%. Perplexity Comet (M = 1.89, green #1DB954): score 0 = 6.9%, score 1 = 26.7%, score 2 = 43.5%, score 3 = 17.6%, score 4 = 4.6%, score 5 = 0.8%, score 6 = 0.0%. No agent from any platform scored 6/6. Mariner and Claude cluster near zero, while ChatGPT and Perplexity show higher but still predominantly sub-threshold scores.*
...
*Three screenshots showing different agent behaviors during survey completion. Panel 1: ChatGPT Agent displaying a visible mouse cursor moving across the survey screen, unlike other agents that programmed clicks into specific browser coordinates. This mouse pointer capability may trivialize mouse movement tracking as a detection tool. Panel 2: Perplexity Comet typing word-by-word via simulated keyboard input rather than copy-pasting text into form fields, making its interaction patterns more human-like. Panel 3: Claude Sonnet 4.5 via Cursor Agent also demonstrating word-by-word simulated keyboard typing, similar to Perplexity, indicating that blocking copy/paste alone would not stop all agents.*

*Note: Figure images not included in machine-readable version. See published paper for the visuals.*
...
*Horizontal Cleveland dot plot (dumbbell plot) with average pass rate (%) on the x-axis (range -5% to 100%) and 12 base models on the y-axis, sorted by magnitude of thinking effect (largest gains at top). For each model, a circle marks the standard variant and a diamond marks the extended-thinking variant, connected by a horizontal line. Green lines (#1a9641) indicate improvement, red lines (#c44e2d) indicate decline. Difference annotations in bold text appear to the right of each pair. The 12 thinking pairs plotted are: Gemini 2.5 Pro, Gemini 2.5 Flash, Gemini 2.5 Flash Lite, GPT-5, Claude Sonnet 4.5, Claude Haiku 4.5, Claude Opus 4.5, GPT-5.1, Gemini 3 Flash, GPT-5.2, Claude Opus 4.6, and Claude Sonnet 4.6. A dashed red vertical line marks the 86.8% human baseline, which no thinking model reaches. The best thinking model (Gemini 3 Flash with thinking, 65.0%) remains 21.8 pp below human performance. Eight of 12 pairs show improvement (mean +8.8 pp), four show declines. The largest gain is Claude Sonnet 4.5 (+30.0 pp); the largest decline is GPT-5.1 (-20.0 pp).*
...
*Two-panel bar chart comparing four detection methods side by side. Left panel titled "Sensitivity" shows agent detection rate (%); right panel titled "Specificity (lower = better)" shows false positive rate (%). Bar colors encode performance: green for best, yellow/amber for intermediate, red for worst. The exact values plotted in the left panel (detection rates) are: Cognitive Traps = 97.1% (green), CAPTCHA optimal = 93.9% (yellow), CAPTCHA standard = 4.6% (red), Traditional Methods = 2.3% (red). The exact values plotted in the right panel (false positive rates) are: Cognitive Traps = 4.1% (green), CAPTCHA optimal = 12.4% (yellow), CAPTCHA standard = 1.0% (green), Traditional Methods = 11.0% (red). Bold percentage labels appear above each bar. Cognitive traps achieve the best combined profile: highest sensitivity (97.1%) with low false positives (4.1%). CAPTCHA at the optimal 0.90 threshold approaches cognitive trap sensitivity (93.9%) but at three times the false positive rate (12.4% vs. 4.1%).*
...
*Four-panel (2x2) bar chart comparing Prolific (blue, #2c7bb6), MTurk (yellow, #fdae61), Connect (green, #1a9641), and Agents (red, #d7191c) across four indicators. Panel A (Cognitive Trap Flagging): exact values plotted are Prolific 4.1%, MTurk 76.0%, Connect 1.8%, Agents 97.1%. Y-axis 0-110%. Panel B (reCAPTCHA v3 Scores): exact values plotted are Prolific 0.972, MTurk 0.635, Connect 0.984, Agents 0.833. Y-axis 0.4-1.08, with a dashed horizontal line at the 0.5 standard threshold. Panel C (Choice Deferral by Condition): paired bars for conflict and dominated conditions. Conflict condition values: Prolific 24.0%, MTurk 1.6%, Connect 26.6%, Agents 0.7%. Dominated condition values: Prolific 5.2%, MTurk 0.0%, Connect 6.7%, Agents 0.0%. Darker bars show conflict, lighter bars show dominated. Y-axis 0-35%. Panel D (Traditional Attention Checks): three grouped bars per platform for IMC, Obvious Answer, and Memory Check. IMC pass rates: Prolific 99.5%, MTurk 84.4%, Connect 99.4%, Agents 99.8%. Obvious Answer pass rates: Prolific 99.0%, MTurk 55.9%, Connect 100.0%, Agents 99.8%. Memory Check pass rates: Prolific 90.4%, MTurk 60.5%, Connect 91.3%, Agents 99.2%. Y-axis 40-105%.*
...
*Scatter plot with average cost per trial (USD, log scale) on the x-axis and cognitive trap accuracy (%) on the y-axis (range -3% to 95%) for 34 models. Each point colored by provider: Anthropic (rust red, #c44e2d), OpenAI (green, #3a7d5e), Google (blue, #2c5f7c). Standard models are plotted as circles, extended thinking variants as diamonds. A dashed gray regression line is essentially flat (r = .23, p = .18), confirming no relationship between computational cost and accuracy. A dotted blue horizontal line marks the 86.8% human baseline. Six key models are labeled with arrows: Gemini 3 Flash with thinking (best accuracy + cheapest), Claude Opus 4.6 with thinking (most expensive), Gemini 2.5 Flash Lite with thinking (cheap + good), Claude Haiku 3.0 (old + good), GPT-4o (zero accuracy), and GPT-5.2 Pro (expensive + mediocre). Cost spans three orders of magnitude from $0.00005 (Gemini 2.5 Flash Lite) to $0.0476 (Claude Opus 4.6 with thinking), a 950-fold range producing no systematic accuracy advantage.*
